...
Провести интерполяцию функции y = e^x
 на последовательности отрезков [0,2], [0,1], [0,1/2],...,[0,1/16]
. использовать равномерное расположение узлов из N узлов
Построить зависимость ошибки интегрполяции от длины отрезка
Аналогичное провести для Чебышевского расположения узлов
Предыдущие пункты выполнить для N = 3, 4, 5
Замечание. Ошибку интерполяции можно оценить как максимальный модуль разности между интерполянтом
и функцией в 1000 равномерно расположенных точках
...
int main() {
    ...
    // fout << "Ошибка для " << N << " узлов" << std::endl;
    // fout << "Для равномерного распределения | для Чебышевского | отрезок [0; pow(2, k)]" << std::endl;
